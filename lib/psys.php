...
//		'BC' =>		array('Bitcoin',		'BTC', 1, 1, 'BTC', 'itsbtc'),
		...
		'CP' =>		array('Bitcoin (Coinpayments.net)',	'BTC', 1, 1, 'BTC', 'itscp'),
		'AB' =>		array('Bitcoin (ApiBtc.com)',	'BTC', 1, 1, 'BTC', 'apibtc_id'),
...
//			$res = @file_get_contents('https://blockchain.info/api/receive?' . $parameters);
	...
	case 'AB':
		require_once('lib/inet.php');
		$urlproc .= "?itsab=1&tag=$tag&key=$params2[key]";
		$url = "https://apibtc.com/api/create_wallet/?token=$params2[apipass]&callback=" . urlencode($urlproc);
		$res = json_decode(inet_request($url), 1);
		if ($res["success"])
		{
			$sign = md5($res["Res"]["Address"] . $params2['access']);
			if ($sign == $res["Res"]["Sign"])
				return array(
					'url' => "https://apibtc.com/merchant/invoice/?wallet=" . $res["Res"]["Adress"],
					'amount' => $sum,
					'return_user' => $urlok
				);
		}
...
//			'batch' => $arr['transaction_hash'],
	...
	case 'AB':
		...
		$r = array(
			...
			'hash' => $arr['hash'],
			'confirmations' => 0 + $arr['confirmations'],
			'wait' => ($arr['confirmations'] < 2)
		);
		if ($r['wait'])
			exit;
		echo("Successfully|#$tag");
		$r['cc'] = md5($params2['access'] . $arr['txid'] . $arr['apibtc_id'] . $arr['address']);
...
//
	...
	case 'BC':
		...
		return $res;
	case 'AB':
		$res['answ'] = inet_request($req = "https://apibtc.com/api/get_balance/?token=$params[apipass]");
		$answ = json_decode($res['answ'], 1);
		if (is_array($answ) and isset($answ['balance']))
		{
			$res['result'] = 'OK';
			$res['sum'] = $answ['balance'];
		}
		else
			$res['result'] = $res['answ'];
		return $res;
	...
	case 'CKE':
		...
		return $res;
	case 'CP':
		require('lib/cp/coinpayments.php');
...
//
	...
	case 'AB':
		if (!preg_match('|[1-9A-Za-z]{27,34}|', $toparams['acc']))
			return $res;
		$res['answ'] = inet_request("https://apibtc.com/api/sendmoney/?token=$fromparams[apipass]&".
			"wallet=$toparams[acc]&amount=$sum");
		$answ = json_decode($res['answ'], 1);
		if ($answ['success'])
		{
			$res['result'] = 'OK';
			$res['batch'] = $answ['Res']['tx'];
		}
		else
			$res['result'] = $answ['msg'];
		return $res;
	...
	case 'CP':
		...
		$result = $cps->CreateWithdrawal($sum, $c[4], $toparams['acc'], true);
xaddtolog($result, 'cp');
		if ($result['error'] == 'ok')
		{
			$res['result'] = 'OK';
